...
def generate_pdf_report(username, final_score, user_data, hours, attendance, previous, sleep, recommendations):
    ...
    story.append(Spacer(1, 0.2*inch))
    story.append(Paragraph(ParagraphStyle('Footer', parent=styles['Normal'], fontSize=8, textColor=colors.grey, alignment=1)))
...
def show_main_app():
    ...
    if st.button("Predict Score", use_container_width=True):
        ...
        if len(user_history) >= 1:
            ...
            bar_df = pd.DataFrame({
                'Category': ['Below 60', '60 - 80', 'Above 80'],
                'Count': [below_60, between_60_80, above_80]
            })
            
            fig_bar = px.bar(bar_df, x='Category', y='Count', 
                            title='Score Distribution',
                            text='Count',
                            color_discrete_sequence=['#00adb5', '#764ba2', '#f39c12'])
            fig_bar.update_traces(textposition='outside', marker_line_color='#007a7f', marker_line_width=1)
